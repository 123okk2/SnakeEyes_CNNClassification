...
#------1. 데이터 셋 불러오기
#train 셋 : 100만 개의 데이터
train = np.vstack(tuple(np.fromfile("snake-eyes/snakeeyes_{:02d}.dat".format(i), dtype=np.uint8).reshape(-1,401) for i in range(10)))
x_train = train[:, 1:]
y1 = train[:, 0]
...
y_train = np.zeros([len(y1), 12], dtype=bool)
for i in range(len(y1)):
    label = y1[i]
    y_train[i, label - 1] = True
...
n=random.sample(range(0,10000),20)
for k in range(1, 21):
    plt.subplot(2, 10, k)
    plt.title('LeNet5_Result')
    plt.imshow(x_test[n[k-1]].reshape(20,20))
    plt.title(y1[n[k-1]])
    plt.axis('off')
    plt.axis('off')
plt.show()
...
model.fit({'input': x_train}, {'target': y_train}, n_epoch=epoch, validation_set=({'input': x_test}, {'target': y_test}), snapshot_step=100, show_metric=True, run_id='snakeEyes')

model.save('TestModels/LeNet5Model')
...
model.fit(x_train, y_train, n_epoch=epoch, validation_set=(x_test,y_test), show_metric=True, batch_size=500, snapshot_step=100, run_id='snakeEyes')

model.save('TestModels/AlexNetModel')
'''
'''
#VGGNet 실험
...
model.fit(x_train, y_train, n_epoch=epoch, validation_set=(x_test,y_test), show_metric=True, batch_size=500, snapshot_step=100, snapshot_epoch=False, run_id='snakeEyes')

model.save('TestModels/VGGNetModel')
'''
'''
#ResNet 실험
...
model.fit(x_train, y_train, n_epoch=epoch, validation_set=(x_test,y_test), show_metric=True, batch_size=500, run_id='snakeEyes')

model.save('TestModels/ReNet5')
...
model.fit(x_train, y_train, n_epoch=epoch, validation_set=(x_test,y_test), shuffle=True, show_metric=True, batch_size=64, snapshot_step=200, snapshot_epoch=False, run_id='snakeEyes')

model.save('TestModels/GoogleNet')
